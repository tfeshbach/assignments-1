...
struct ppm_pixel *mandelbrot (int size, float xmin, float xmax, float ymin, float ymax, 
int maxIterations, struct ppm_pixel *palette, struct ppm_pixel black,
struct ppm_pixel *grid){

struct ppm_pixel color;
for (float i = 0; i < size; i++) { //for every row in the image
   for (float j = 0; j < size; j++) { //for each col in the image 
      float xfrac = i / (size);
      float yfrac = j / (size);
...
int main(int argc, char* argv[]) {
  ...
  grid = mandelbrot(size, xmin, xmax, ymin, ymax, maxIterations, palette, black, grid);
