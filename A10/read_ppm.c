...
struct ppm_pixel* read_ppm(const char* filename, int* w, int* h) {
  ...
  int going = 0; //check if we have more header

  while (going == 0) { //if we're going
    char str[1000]; //space for line
    fgets(str, 1000, infile); //read in line
    ...
    if ((startsWith(str, firstLine) == 0) && (startsWith(str, comment) == 0) && (startsWith(str, max) == 0)){ //if line doesn't start w/above
      ...
      char height[50] = "";
      int i = 0;
      while (str[i] != '\n' || i > strlen(str)) {  //for every char in the entire line
    ...
    if ((startsWith(str, max) == 1)) {
          going = 1; //no more header
        }
  }
